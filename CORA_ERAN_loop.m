...
model_file='example_nonlinear_reach_TORA_u.m';
...
% run simulation
%figure
run('/home/antonio/Workspace/Work_Verimag/hscc_2019_implementation/hscc_implementation/neural_network_reachability/examples/systems_with_networks/Ex_Tora/simulate_with_NN.m')
...
switch model_short_name
    case 'TORA'
        initial_ranges=[0.6 0.61; -0.7 -0.69; -0.4 -0.39; 0.59 0.6; 2 300];
end
ranges{1}=initial_ranges;%python3 . --netname ../nets/bigger_controller.tf --domain refinezono --input text
timerStart = tic;
...
for iteration=1:iterations
    ...
    if iteration==1
        ...
        %Assuming one control variable u
        NN_lb{iteration} = 0;
        NN_ub{iteration} = 0;
        ...
        cd(current_PATH)
        cd CORA_models
        lb(end)=NN_lb{iteration};
        ub(end)=NN_ub{iteration};
        switch model_short_name
            case 'TORA'
                %if it==1
                %     [R,opt,~]=example_nonlinear_reach_TORA();
                % else
                [R,opt,~]=example_nonlinear_reach_TORA_u(0,0,lb,ub);
                % end
        end
        ...
        if ~zonotope_output
            % Interval Inputs:
            % We need lower and upper bR_valuesounds for each variable that is
            ...
            disp('')
            R_zono
            %VERY SMART WAY TO GET RID OF A DIMENSION
            R_values_eran=R_values(1:(size(R_values,1)-1),:);
            R_zono_eran=zonotope(R_values_eran);
            disp('The final smaller zonotope is described below.')
            disp('')
            R_zono_eran
            % end
            ...
            no_outputs=length(ub)-1;
            fprintf(fid,'%i\n',no_outputs);
            no_generators=size(R_values_eran,2)-1;
            fprintf(fid,'%i\n',no_generators+1);
            matrix_zono=R_values_eran';
        ...
        if ~zonotope_output
            ...
            switch model_short_name
                case 'TORA'
                    ...
                    %     [R,opt,~]=example_nonlinear_reach_TORA();
                    % else
                    %R_zono 5 dimensions & old u, R_zono_eran 4 dimesnions,
                    % NN_lb{iteration}, NN_ub or lb(end) and ub(end)
                    % R_concatenated= R_zono_eran & ub,lb(end)
                    R_concatenated=[R_values_eran, zeros(size(R_values_eran,1),1);(ub(end)+lb(end))/2, zeros(1,size(R_values_eran,2)-1),(ub(end)-lb(end))/2]; 
                    [R,opt,~]=example_nonlinear_reach_TORA_u(0,0,[],[],zonotope(R_concatenated));
                    % end
            end
            % keep reachable set at the last time instance
            R_final_zonotope=R{end}{1};
